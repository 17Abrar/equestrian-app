...
must already exist in _Settings → Integrations_ before the rule's
action will resolve.
...
use this template at _Project → Alerts → Create Alert Rule → Issues_:
...
**How to add the filter** — Cloudflare dashboard: _Workers & Pages →
your worker → Logs → Logpush → Add destination → filter by
`$.event == "cron_scheduled_failed"`_. Do this once per event name
