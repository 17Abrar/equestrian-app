...
Audit LOW-14 (2026-05-05): the .env.example template is short on the _why_
...
slots; this file explains what each one _does_ and what breaks if you skip
...
- **Why:** Cavaliq's **own** Ziina merchant account that bills clubs
  ...
  from the per-club Ziina keys above — those receive _rider_ payments;
  this one receives the _club's subscription_ payment.
...
- **Why:** drives the Ziina sandbox `test` flag on every platform
  ...
  a sandbox `PLATFORM_ZIINA_API_KEY` will issue _live_ intents that
