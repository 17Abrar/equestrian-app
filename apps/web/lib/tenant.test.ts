...
describe('getTenantContext — Path 1 (Clerk active org)', () => {
  it('returns the member-role context when org + active membership both resolve', async () => {
    ...
    // member lookup
    limitMock.mockResolvedValueOnce([{ id: MEMBER_A, role: 'club_admin', isActive: true }]);
  ...
  it('throws NO_MEMBERSHIP — never falls back to Clerk JWT role — when club resolves but no member row exists at all (webhook race)', async () => {
    ...
    // Pass-3 follow-up (2026-05-18 codex pass-3): NO_MEMBERSHIP now means
    // strictly "no row" (transient webhook race) — see paired
    // MEMBERSHIP_DEACTIVATED test below for the row-exists-inactive case.
    authMock.mockResolvedValue({ userId: USER_ID, orgId: ORG_ID });
    limitMock.mockResolvedValueOnce([{ id: CLUB_A, onboardingCompletedAt: new Date() }]);
    limitMock.mockResolvedValueOnce([]); // no row at all
  ...
  it('throws MEMBERSHIP_DEACTIVATED — distinct from NO_MEMBERSHIP — when the member row exists but is_active=false', async () => {
    // Pass-3 codex follow-up (2026-05-18): differentiating the two
    // refusal cases so the /onboarding placeholder no longer auto-
    // refreshes a permanently-deactivated user forever. The previous
    // shape collapsed both into NO_MEMBERSHIP, which was safe (refuses
    // either) but produced misleading UX downstream.
    authMock.mockResolvedValue({ userId: USER_ID, orgId: ORG_ID });
    limitMock.mockResolvedValueOnce([{ id: CLUB_A, onboardingCompletedAt: new Date() }]);
    limitMock.mockResolvedValueOnce([{ id: MEMBER_A, role: 'club_admin', isActive: false }]);
