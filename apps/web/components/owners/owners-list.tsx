...
export function OwnersList({ canCreate = true }: OwnersListProps = {}) {
  ...
  if (isError)
    return (
      <ErrorState
        message={error instanceof Error ? error.message : 'Failed to load owners'}
        onRetry={() => refetch()}
      />
    );
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Manage horse owners and their profiles</p>
        </div>
      ...
      {owners.length === 0 && (
        <EmptyState
          ...
          action={canCreate ? { label: 'Add Owner', onClick: () => setAddOpen(true) } : undefined}
        />
      )}
                  ...
                  <AlertDialog>
                    ...
                    <AlertDialogContent>
                      <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                          This will deactivate their account. Their horses will remain in the
                          system.
                        </AlertDialogDescription>
                      </AlertDialogHeader>
                      <AlertDialogFooter>
                        <AlertDialogCancel>Cancel</AlertDialogCancel>
                        <AlertDialogAction onClick={() => handleDeactivate(owner.id)}>
                          Remove
                        </AlertDialogAction>
                      </AlertDialogFooter>
                    </AlertDialogContent>
...
function AddOwnerDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Add Horse Owner</DialogTitle>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            <PersonContactFields control={form.control} emailPlaceholder="owner@example.com" />
