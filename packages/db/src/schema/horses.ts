...
import { horseStatusEnum, skillLevelEnum, horseSaleStatusEnum, ownershipStatusEnum } from './enums';
...
export const horses = pgTable(
  ...
  (table) => [
    ...
    index('idx_horses_livery_billing_due')
      ...
      .where(
        sql`ownership_status = 'active' AND COALESCE(monthly_livery_fee_minor, 0) > 0 AND deleted_at IS NULL`,
      ),
