...
describe('joinClubInstantly', () => {
  ...
  it('reactivates a member who previously left voluntarily (is_active=false, no admin stamp)', async () => {
    ...
    const isMemberBefore = await withTestDb(testDb.db, () => isUserMember(clubId, 'user_rejoin'));
    ...
    const isMemberAfter = await withTestDb(testDb.db, () => isUserMember(clubId, 'user_rejoin'));
  ...
  it('concurrent join attempts resolve to a single membership', async () => {
    ...
    const ids = new Set(results.flatMap((r) => (r.status === 'joined' ? [r.member.id] : [])));
    ...
    const allRows = await testDb.db
      .select()
      .from(clubMembers)
      .where(and(eq(clubMembers.clubId, clubId), eq(clubMembers.clerkUserId, 'user_concurrent')));
    expect(allRows).toHaveLength(1);
  });
  ...
  it('refuses rejoin for an admin-deactivated member (deactivatedByAdminAt set)', async () => {
    ...
    await withTestDb(testDb.db, () => deactivateMember(clubId, initialJoin.member.id));
    ...
    const stored = await testDb.db
      .select()
      .from(clubMembers)
      .where(and(eq(clubMembers.clubId, clubId), eq(clubMembers.clerkUserId, 'user_kicked')));
