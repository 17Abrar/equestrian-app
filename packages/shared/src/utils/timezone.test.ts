...
describe('parseDateTimeLocal', () => {
  ...
  describe('DST transitions', () => {
    it('handles US Eastern spring-forward (2026-03-08 02:00 → 03:00)', () => {
      ...
      expect(
        parseDateTimeLocal('2026-03-08T01:30', 'America/New_York').toISOString(),
      ).toBe('2026-03-08T06:30:00.000Z');
      // 03:30 EDT is UTC-4 → 07:30 UTC (clock has jumped forward).
      expect(
        parseDateTimeLocal('2026-03-08T03:30', 'America/New_York').toISOString(),
    ...
    it('handles US Eastern fall-back (2026-11-01 02:00 → 01:00)', () => {
      ...
      expect(
        parseDateTimeLocal('2026-11-01T00:30', 'America/New_York').toISOString(),
      ).toBe('2026-11-01T04:30:00.000Z');
      // 03:30 EST (after the second 01:30) is UTC-5 → 08:30 UTC.
      expect(
        parseDateTimeLocal('2026-11-01T03:30', 'America/New_York').toISOString(),
    ...
    it('handles Europe/London spring-forward (2026-03-29 01:00 → 02:00)', () => {
      ...
      expect(
        parseDateTimeLocal('2026-03-29T00:30', 'Europe/London').toISOString(),
      ).toBe('2026-03-29T00:30:00.000Z');
      // 02:30 BST is UTC+1 → 01:30 UTC (clock skipped 01:00–01:59).
      expect(
        parseDateTimeLocal('2026-03-29T02:30', 'Europe/London').toISOString(),
    ...
    it('handles Europe/London fall-back (2026-10-25 02:00 → 01:00)', () => {
      ...
      expect(
        parseDateTimeLocal('2026-10-25T00:30', 'Europe/London').toISOString(),
      ).toBe('2026-10-24T23:30:00.000Z');
      // 03:30 GMT (after second 01:30) → 03:30 UTC.
      expect(
        parseDateTimeLocal('2026-10-25T03:30', 'Europe/London').toISOString(),
    ...
    it('Asia/Dubai stays at UTC+4 across all of 2026 (no DST)', () => {
      ...
      expect(
        parseDateTimeLocal('2026-03-29T02:30', 'Asia/Dubai').toISOString(),
      ).toBe('2026-03-28T22:30:00.000Z');
      expect(
        parseDateTimeLocal('2026-10-25T02:30', 'Asia/Dubai').toISOString(),
