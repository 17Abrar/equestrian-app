...
**Optional tuning vars** (audit F-52, 2026-05-07 r5 — not enumerated
in the verification list below because they're operational knobs, not
credentials):

- `WEBHOOK_STALE_AFTER_MS` — claim-recovery window for `received`
  webhook events that crashed mid-processing. Default 300_000 (5 min).
  See `ENV.md` § Cron + Webhooks. `wrangler secret put` if you want to
  override per-env; safe to leave unset.
- `PLATFORM_ZIINA_TEST_MODE` — set to `true` ONLY on staging / preview
  workers. Drives the Ziina sandbox `test` flag for the platform
  payment-intent create. Production must leave it unset (or `false`)
  — `assertProductionEnvConfigured` warns if a `true` value leaks
  into prod. See **Environments** below for the staging template.

**Verifying secrets per release (audit F-75, 2026-05-07 r4):** the
`apps/web/cloudflare-env.d.ts` file is autogenerated by `wrangler types`
and ONLY enumerates `vars` — Worker secrets are intentionally excluded.
Before each deploy, verify the secret list with:

```sh
wrangler secret list cavaliq
```

The expected names are: `DATABASE_URL`, `DATABASE_URL_UNPOOLED`,
`CLERK_SECRET_KEY`, `CLERK_WEBHOOK_SECRET`,
`NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY`, `R2_ENDPOINT`, `R2_ACCESS_KEY_ID`,
`R2_SECRET_ACCESS_KEY`, `R2_BUCKET_NAME`, `R2_PUBLIC_URL`,
`UPSTASH_REDIS_REST_URL`, `UPSTASH_REDIS_REST_TOKEN`, `RESEND_API_KEY`,
`EMAIL_FROM`, `SENTRY_DSN`, `NEXT_PUBLIC_SENTRY_DSN`, `CRON_SECRET`,
`ENCRYPTION_KEY` (+ payment-platform secrets when configured:
`PLATFORM_ZIINA_API_KEY`, `PLATFORM_ZIINA_WEBHOOK_SECRET`).
