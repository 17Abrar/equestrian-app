...
- Use soft deletes per `DATABASE.md` (Deletion patterns table). Audit F-41 (2026-05-08 r6) — there are THREE distinct idioms (`deleted_at` timestamp on `clubs`/`horses`; status-enum transition on `bookings`/`invoices`/`livery_invoices`/`platform_subscription_invoices`/`coupons`; deactivation flag `is_active` on `club_members`/`arenas`/`lesson_types`). New tables MUST pick one of these three; do not invent a fourth. Audit pass-7 (2026-05-24): the prior single "append-only" list conflated two different cases, so it has been split. **(a) Truly INSERT-only** (no `.delete()` in source, no UPDATE except idempotent flag flips like `notified` or vote-count aggregates): `audit_log`, `webhook_events`, `community_votes`, `horse_pairing_history`, `coupon_usages`, `competition_results`, `rider_achievements`, `waitlist`, `burned_webhook_secret_hashes` — these have NO delete pattern by design. **(b) Cascade-only or marker-clear delete** (no independent delete pattern, but rows DO disappear under a specific parent operation): `horse_medication_logs` + `horse_documents` (cascade-deleted when their horse is hard-deleted in `deleteHorse`, see `packages/db/src/queries/horses.ts`), `horse_care_reminder_sends` (cleared by `unrecordHorseCareReminderSend()` when the underlying care event is rescheduled, so the reminder re-fires). New tables MUST land in one of the three idioms above or document why they fit (a)/(b).
